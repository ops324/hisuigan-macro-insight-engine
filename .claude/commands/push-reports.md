...
### Step 1.7: 予測ログをレビューして自己補正する（フィードバックループ）

Step 2 でスタンス・シナリオを生成する**前に**、過去の予測の的中傾向を確認して系統的な偏りを補正する。

1. `content/history/predictions.json` を読み込み、`outcome` が `null` でない（評価済み）エントリのうち直近 8 件程度を確認する
2. 系統的バイアスがないか分析する：
   - **方向バイアス**：ベースシナリオの方向が実績と一貫してズレていないか（例：neutral / down を出し続けているのに実際は上昇が続いている → 弱気バイアス）
   - **スタンスバイアス**：stance が実績に対して一貫して高すぎ／低すぎないか（例：守り重視を続けているが市場は上昇基調）
3. バイアスが認められる場合のみ、今回の stance・シナリオ方向・確率配分に反映して補正する
4. 補正を行った場合は `stanceRationale` にその旨を 1 文添える（例：「直近の予測が下方バイアス気味だった点を踏まえ、確率配分を中立寄りに調整」）

#### 過剰適合の回避（必須）
- **直近 1〜2 件の結果に引きずられない**。週次・日次の方向は短期的にほぼランダムであり、外れた直後に逆張りで方向を反転させると次で外す（リーセンシーバイアス）
- これは「直近の外れを取り返す」操作ではなく、複数件にわたる**系統的な偏り**だけを補正するキャリブレーション作業である
- 評価済みエントリが 5 件未満のときは参考程度にとどめ、大きな補正はしない（サンプル不足）
