...
#### 長期（6M+）の自己点検（必須）
**本プロダクトの主目的は長期（6M+）の予測精度**。日次±1%S&P採点は near-random の短期サニティ層であり、長期スキルの指標ではない。Step 1.7 では以下を点検する：
- **stance の滑らかさ**：stance は中長期(6M+)ゲージである。日次ニュースで乱高下していないか（隣接エントリ間の変化が大きすぎないか）を確認。長期の見立ては日々ほぼ一定であるべき。
- **長期 IC は年単位で成熟**：6M 前方リターンの独立観測は年に数個しか得られないため、現時点では結果ベースの IC をもって stance を動かさない。暫定の短窓 IC（重複窓・有効N僅少）は参考にしない。
- 補正は**遅い構造変数の変化**（バリュエーション・政策サイクル・信用スプレッド等）に基づいて行い、日次イベントだけでは動かさない。
...
| `structuralInputs` | （任意）stance/見立ての根拠となった構造変数のスナップショット（valuation/policy/earnings/credit/curve/trend の所見）。後から hit/miss を構造変数に帰属するための記録 |
